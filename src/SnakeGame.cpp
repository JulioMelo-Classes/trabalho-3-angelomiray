...
bool SnakeGame::setNewLevel(string line){
    ...
    ++totalLevels;
    Levels[totalLevels].setTotalFood(totalFood);
    Levels[totalLevels].setLines(l);
    Levels[totalLevels].setColumns(c);
        ...
        while(getline(levelFile, line)){ //pega cada linha do arquivo
            if(line[0] != '.' && line[0] != '#'){
                if(setNewLevel(line) == false){
                    ...
                    lineCount = 0; //como já foi inicializado o novo level no primeiro if, apenas iniciamos a contagem de linhas dessa nova fase.
                }       
            }
            else{
                Levels[totalLevels].addMazeLine(line);
...
void SnakeGame::update(){
    ...
    switch(state){
        case RUNNING:
            if(frameCount == 0){
                test.find_solution(maze, snake, snake.getSnakeCoordinates());
            }
            else if(frameCount == snake.getSolutionSize()){
                if(snake.sucessSolution() == true && snake.getFoodEaten()+1 != Levels[atualLevel].getTotalFood()){//caso a solução final seja encontrar a fruta e ainda não seja a última fruta do nível
                    snake.eatFood(snake, snake.getPosition(snake.getSolutionSize()-1));
                    snake.clearSolution();
                    test.find_solution(maze, snake, snake.getPosition(snake.getSolutionSize()-1)); //final pos
                }
